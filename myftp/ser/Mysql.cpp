...
int file_name = 1;
char file_path[128] = "/home/stu/File/";
...
bool Register(char *name, char *passwd)
{
	...
	char tablename[100];
	sprintf(tablename,"create table %s(virtual_path varchar(32) not null primary key, real_path varchar(32))",name);
...
int main()
{
	...
	bool tag = false;
	Register("wd","111111");
	tag = remove_file("wd","/","6.cpp");
	printf("tag : %d\n",tag);
	tag = Get_File(result_buff,"wd",buff);
	return 0;
}
