/*************************************************************************
    ...
    > Created Time: 2016年06月10日 星期五 21时35分41秒
 ************************************************************************/
...
public:
	...
	friend istream& operator>>(istream& _I, bitset<_N>& _R)
	{
		...
		if (_Ok)
		{
			...
			for (size_t _M = _R.size(); 0 < _M;_C = _I.rdbuf()->snextc(), --_M)
			{
				if (_C == EOF)
				{
					_St |= ios_base::eofbit;
	...
	private:
	enum {_Nb = CHAR_BIT * sizeof (_Ty),_Nw = _N == 0 ? 0 : (_N - 1) / _Nb};//_Nb:32bit,_Nw:集合能站几个unsign long
	void _Tidy(_Ty _X = 0)
	{
		for (int _I = _Nw; 0 <= _I; --_I)
			_A[_I] = _X;
